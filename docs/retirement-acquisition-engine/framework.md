...
### Locked Operating Framework — v2.11
...
1. **If `stated_intent == mainly_researching` → `education`.** Intent takes precedence here even when `written_plan_status == has_current_plan`. A trusted plan does not create second-opinion intent.
2. **Else, if `written_plan_status == has_current_plan` AND `stated_intent == second_opinion` → `second_opinion`.** Second Opinion requires both an existing current/trusted plan and explicit second-opinion intent.
3. **Else, if `written_plan_status == has_current_plan` → `education`.** The household may still receive a Review CTA from Fit Tier or a High-Intent Event, but Planning Track itself stays neutral rather than inventing a second-opinion request.
4. **Else,** the plan-status default applies: `no_plan` / `accounts_no_plan` → `build_first_plan`; `projections_only` / `outdated_plan` → `update_coordinate`.
...
|---|---|---|
| Stress-Test Build Version | Text | v2.11 (latest deployed preview/schema sync; Fit and Engagement models unchanged) |
